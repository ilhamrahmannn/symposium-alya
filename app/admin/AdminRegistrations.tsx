...
import { CalendarClock, Check, Download, Eye, FileDown, RotateCcw, Search, Trash2, X } from "lucide-react";
...
type Registration={
  ...
  verifiedBy?:string;linkedTransactionId?:string;testRecord?:boolean;
  reminderStatus?:"sent"|"failed";reminderSentAt?:FirestoreDate;reminderAttemptedAt?:FirestoreDate;reminderError?:string;reminderAttempts?:number;
};

export default function AdminRegistrations({notify}:{notify:(message:string)=>void}) {
  const [rows,setRows]=useState<Registration[]>([]),[search,setSearch]=useState(""),[payment,setPayment]=useState("all"),[status,setStatus]=useState("all"),[selected,setSelected]=useState<Registration|null>(null),[error,setError]=useState(""),[reminderAction,setReminderAction]=useState<"unsent"|"failed"|null>(null);
  ...
  const confirmedRows=rows.filter(row=>row.registrationStatus==="confirmed"&&row.testRecord!==true);
  const reminderSentCount=confirmedRows.filter(row=>row.reminderStatus==="sent").length;
  const reminderFailedCount=confirmedRows.filter(row=>row.reminderStatus==="failed").length;
  const reminderUnsentCount=confirmedRows.filter(row=>row.reminderStatus!=="sent").length;
  ...
  const sendReminders=async(mode:"unsent"|"failed")=>{
    const targetCount=mode==="failed"?reminderFailedCount:reminderUnsentCount;
    const label=mode==="failed"?"failed reminder":"confirmed participant without a recorded successful reminder";
    if(!auth?.currentUser||targetCount===0||!confirm(`Send the event reminder to ${targetCount} ${label}${targetCount===1?"":"s"}? Participants marked Reminder Sent will be skipped.`))return;
    setReminderAction(mode);setError("");
    try{
      const result=await requestEventReminder(auth.currentUser,mode);
      notify(`Event reminder sent to ${result.sent} of ${result.total}; ${result.failed} failed${result.skipped?`; ${result.skipped} previously sent and skipped`:""}${result.trackingFailed?`; ${result.trackingFailed} tracking updates failed`:""}`);
    }catch(cause){setError(cause instanceof Error?cause.message:"Event reminder could not be sent.")}
    finally{setReminderAction(null)}
  };
  const csv=()=>{
    ...
    const headers=[
      ...
      "Verification Date","Verified By","Linked Transaction ID","Reminder Status","Reminder Sent At","Reminder Attempted At","Reminder Error","Reminder Attempts",
    ];
    const records=filtered.map(row=>[
      ...
      date(row.verifiedAt),row.verifiedBy||"",row.linkedTransactionId||"",row.reminderStatus||"",date(row.reminderSentAt),date(row.reminderAttemptedAt),row.reminderError||"",row.reminderAttempts||0,
    ]);
