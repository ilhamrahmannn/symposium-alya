...
export default function AdminRegistrations({notify}:{notify:(message:string)=>void}) {
  ...
  const removeRejected=async(row:Registration)=>{
    if(!db||!auth?.currentUser||row.registrationStatus!=="rejected"||!confirm(`Permanently delete rejected registration ${row.referenceNumber}? This cannot be undone.`))return;
    try {
      await runTransaction(db,async transaction=>{
        const registrationRef=doc(db,"programs",PROGRAM_ID,"registrations",row.id);
        const snapshot=await transaction.get(registrationRef);
        const current=snapshot.data() as Registration|undefined;
        if(!current||current.registrationStatus!=="rejected"||current.paymentStatus!=="rejected"||current.linkedTransactionId)throw new Error("Only unlinked rejected registrations can be deleted.");
      ...
      notify("Rejected registration deleted and its seat released");
      setSelected(null);
    } catch { setError("The rejected registration could not be deleted. No capacity changes were saved."); }
  };
  ...
  const actions=(row:Registration,mobile=false)=><>
    ...
    {row.registrationStatus==="rejected"&&<button className={mobile?"reject wide":"row-menu danger"} onClick={()=>removeRejected(row)} aria-label="Delete rejected registration"><Trash2/>{mobile&&"Delete Rejected Registration"}</button>}
  </>;
