...
import { ArrowRight, Bone, Brain, HeartPulse, Sparkles, Stethoscope, Users, X } from "lucide-react";
...
const disciplines=[
  {icon:HeartPulse,label:"Neonatology and Paediatrics",speakers:[
    {name:"Dr Chia Le Ser",role:"Paediatric Neonatology Consultant",organisation:"Hospital Sultan Ismail, Johor Bahru",image:"/speakers/speaker-3.webp"},
    {name:"Dr Liew Zheyi",role:"Paediatric Respiratory Specialist",organisation:"Hospital Sultanah Aminah, Johor Bahru",image:"/speakers/speaker-5.webp"},
  ]},
  ...
  {icon:Stethoscope,label:"Otorhinolaryngology",speakers:[
    {name:"Dr Selvamalar Vengathajalam",role:"Paediatric Otorhinolaryngology Specialist",organisation:"Hospital Pulau Pinang",image:"/speakers/speaker-4.webp"},
  ]},
  ...
  {icon:Users,label:"Other related healthcare disciplines",speakers:[
    {name:"Cik Charmien Sim Siaw Yuin",role:"Dental Technician",organisation:"Hospital Sultan Ismail, Johor Bahru",image:"/speakers/speaker-8.webp"},
  ]},
] as const;
...
export default function EventHomepage(){
  ...
  useEffect(()=>{if(!selectedDiscipline)return;const previous=document.body.style.overflow;document.body.style.overflow="hidden";const escape=(event:KeyboardEvent)=>{if(event.key==="Escape"){setSelectedDiscipline(null);requestAnimationFrame(()=>lastTrigger.current?.focus())}};window.addEventListener("keydown",escape);return()=>{document.body.style.overflow=previous;window.removeEventListener("keydown",escape)}},[selectedDiscipline]);
  const closeSpeakers=()=>{setSelectedDiscipline(null);requestAnimationFrame(()=>lastTrigger.current?.focus())};
